...
fn main() -> anyhow::Result<()> {
    ...
    println!(
        "Initial GPU memory: {:.0} MB ({:.3} GB)",
        get_gpu_memory_mb(),
        get_gpu_memory_mb() / 1024.0
    );
    ...
    println!(
        "VAE loaded. GPU memory: {:.0} MB ({:.3} GB)",
        get_gpu_memory_mb(),
        get_gpu_memory_mb() / 1024.0
    );
    ...
    .to_dtype(DType::BF16)?;
    println!(
        "After creating latents: {:.0} MB ({:.3} GB)",
        get_gpu_memory_mb(),
        get_gpu_memory_mb() / 1024.0
    );
    ...
    println!(
        "After dropping tensors: {:.0} MB ({:.3} GB)",
        get_gpu_memory_mb(),
        get_gpu_memory_mb() / 1024.0
    );
