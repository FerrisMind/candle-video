//! Wan VAE step-by-step parity test - compares Rust decoder blocks with Python diffusers.
//!
//! Run: cargo test verify_wan_vae_stepwise --release -- --nocapture
    ...
    #[cfg(feature = "cuda")]
    {
        if let Ok(device) = Device::cuda_if_available(0) {
            if let Device::Cuda(_) = device {
                // Try to get memory info via nvidia-smi or similar
...
#[test]
fn verify_wan_vae_stepwise_decode() {
    // Skip if reference file doesn't exist
    if !Path::new(REF_FILE).exists() {
        println!("Skipping test: {} not found", REF_FILE);
        println!("Run: python scripts/gen_wan_vae_stepwise.py");
        return;
    }
    ...
    println!("\n=== VAE Full Decode Parity Results ===");
...
#[test]
fn verify_wan_vae_intermediate_shapes() {
    // Skip if reference file doesn't exist
    if !Path::new(REF_FILE).exists() {
        println!("Skipping test: {} not found", REF_FILE);
        return;
    ...
    let shape_checks = [
        ...
        ("frame0_decoder_up_block_0", vec![1, 192, 1, 120, 120]),
        ("frame0_decoder_up_block_1", vec![1, 192, 1, 240, 240]),
        ("frame0_decoder_up_block_2", vec![1, 96, 1, 480, 480]),
        ("frame0_decoder_up_block_3", vec![1, 96, 1, 480, 480]),
        ("frame0_decoder_norm_out", vec![1, 96, 1, 480, 480]),
        ("frame0_decoder_conv_out", vec![1, 3, 1, 480, 480]),
    ];
    ...
    for (name, expected_shape) in shape_checks_f1 {
        if let Some(tensor) = load_reference_tensor(name) {
            let actual_shape: Vec<usize> = tensor.dims().to_vec();
            println!("{}: {:?}", name, actual_shape);
            assert_eq!(actual_shape, expected_shape, "Shape mismatch for {}", name);
