...
mod tests {
    ...
    fn test_simple_conv3d() -> anyhow::Result<()> {
        ...
        // Create Conv3d
        let config = Conv3dConfig::new((2, 2, 2)).with_padding((0, 0, 0));
        ...
        println!(
            "Rust output: {:?}",
            rust_output.flatten_all()?.to_vec1::<f32>()?
        );
        println!(
            "Python output: {:?}",
            ref_output.flatten_all()?.to_vec1::<f32>()?
        );
    ...
    fn test_im2col_manual() -> anyhow::Result<()> {
        ...
        // Verify
        for (i, (e, a)) in expected_first_patch
            .iter()
            .zip(cols_vec[0][0].iter())
            .enumerate()
        {
            assert!(
                (e - a).abs() < 1e-6,
