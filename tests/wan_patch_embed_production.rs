...
fn patch_embedding_production_shape_matches_fixture() {
    ...
    let dir = PathBuf::from("/home/mod479711/Downloads/Wan2.1-T2V-1.3B-Diffusers/transformer");
    ...
    let patch = WanPatchEmbedding::new(16, 1536, [1, 2, 2], vb.pp("patch_embedding")).expect("patch");
    ...
    let expected = [-0.2295476645231247f32, 0.1298161745071411, 0.045674264430999756, -0.26436924934387207];
    for (i, (&e, &g)) in expected.iter().zip(row.iter()).enumerate() {
        eprintln!("patch dim {i}: ref={e} got={g} diff={}", (e-g).abs());
    }
    let max_diff = expected.iter().zip(row.iter()).map(|(e,g)| (e-g).abs()).fold(0f32, f32::max);
