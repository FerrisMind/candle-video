...
fn main() -> anyhow::Result<()> {
    ...
    let t5_path = format!("{}/text_encoder_gguf/t5-v1_1-xxl-encoder-Q5_K_M.gguf", model_path);
    ...
    let neg_encoding = tokenizer.encode(negative_prompt, true).map_err(|e| anyhow::anyhow!(e))?;
    ...
    if let Ok(vals) = prompt_embeds.flatten_all()?.to_vec1::<f32>() {
        ...
        println!("Prompt embeddings mean: {:.6}", vals.iter().sum::<f32>() / vals.len() as f32);
    }
    ...
    let negative_prompt_attention_mask_cpu = negative_prompt_attention_mask.to_device(&Device::Cpu)?;
    ...
    let nm_data = negative_prompt_attention_mask_cpu.flatten_all()?.to_vec1::<f32>()?;
