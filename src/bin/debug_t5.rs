...
fn main() -> anyhow::Result<()> {
    ...
    if let Ok(vals) = embeddings.flatten_all()?.to_vec1::<f32>() {
        ...
        let variance: f32 = vals.iter().map(|x| (x - mean).powi(2)).sum::<f32>() / vals.len() as f32;
    ...
    println!("Token embeddings first 10: [1.617, 4.469, -3.484, 1.070, 2.031, 0.660, 4.531, -0.676, 0.124, -1.453]");
