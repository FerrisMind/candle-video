    ...
    fn compute_scale_shift_gates(
        ...
        match temb_f.rank() {
            3 => {
                ...
                for t in parts {
                    p.push(t.squeeze(1)?.unsqueeze(1)?);
                }
                Ok((
                    p[0].clone(),
                    p[1].clone(),
...
impl WanTransformer3DModel {
    ...
    pub fn forward(
        ...
        let hs = hs.flatten_from(2)?;
        let mut hs = hs.transpose(1, 2)?;
