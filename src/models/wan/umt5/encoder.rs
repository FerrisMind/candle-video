//! UMT5 encoder (Diffusers `UMT5EncoderModel` parity).
//!
//! Unlike standard T5, every layer has its own `relative_attention_bias` and
//! recomputes position bias instead of reusing layer-0 bias across the stack.
//!
...
impl T5Stack {
    fn load(decoder: bool, vb: VarBuilder, shared: &Arc<Embedding>, cfg: &Config) -> Result<Self> {
        // UMT5: every encoder block has its own relative attention bias weights.
        let block = (0..cfg.num_layers)
            .map(|i| T5Block::load(true, decoder, vb.pp(format!("block.{i}")), cfg))
    ...
    fn forward_dt(
        ...
        for block in self.block.iter_mut() {
            // UMT5 recomputes position bias per layer (does not reuse layer-0 bias).
            (hidden_states, _) = block.forward(&hidden_states, None, encoder_hidden_states)?;
        }
...
#[derive(Debug, Clone)]
pub struct Umt5EncoderModel {
    encoder: T5Stack,
    device: Device,
    span: tracing::Span,
}

impl Umt5EncoderModel {
