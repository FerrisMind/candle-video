...
impl WanTransformer3DModel {
    ...
    pub fn from_var_builder(config: WanTransformerConfig, vb: VarBuilder, device: &Device) -> Result<Self> {
        let inner_dim = config.num_attention_heads * config.attention_head_dim;
        ...
        for i in 0..config.num_layers {
            blocks.push(WanTransformerBlock::new(&config, vb.pp("blocks").pp(i.to_string()))?);
        }
    ...
    pub fn forward(
        ...
        let (cos, sin) = self.rope.forward(hidden_states)?;
        let mut hidden_states = self.patch_embedding.forward(hidden_states)?;

        let (temb, timestep_proj, encoder_hidden_states) =
            self.condition_embedder.forward(timestep, encoder_hidden_states)?;
        ...
        hidden_states = hidden_states
            .reshape((b, out_channels, post_patch_frames * p_t, post_patch_height * p_h, post_patch_width * p_w))?;
