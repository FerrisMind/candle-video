...
pub fn chunked_attention(
    ...
    let num_query_chunks = (seq_q + query_chunk_size - 1) / query_chunk_size;
...
pub fn chunked_attention_kv(
    ...
    let num_kv_chunks = (seq_kv + kv_chunk_size - 1) / kv_chunk_size;
...
mod tests {
    ...
    fn test_chunked_attention_matches_standard() -> Result<()> {
        ...
        let diff = (&standard_output - &chunked_output)?.abs()?.max_all()?.to_scalar::<f32>()?;
