#!/usr/bin/env python3
"""
Profile Wan VAE decoder memory with explicit GC between frames.
"""
...
def main():
    print("=" * 70)
    print("Wan VAE Decoder Memory Profile WITH GC")
    print("=" * 70)
    ...
    print("\n" + "=" * 70)
    print("Decoding with GC after each frame...")
    print("=" * 70)
    
    # Post quant conv
    x = vae.post_quant_conv(z)
    
    # Delete z to free memory
    del z
    gc.collect()
    torch.cuda.empty_cache()
    
    mem_after_pqc = get_gpu_mem_gb()
    print(f"After post_quant_conv + GC: {x.shape}, mem: {mem_after_pqc:.3f} GB")
    ...
    for i in range(latent_f):
        ...
        if i == 0:
            ...
            # Concatenate and immediately delete old tensors
            new_out = torch.cat([out, out_], 2)
            del out
            del out_
            out = new_out
            del new_out
    ...
    print(f"Output tensor size: {get_tensor_size_mb(out):.1f} MB")
